...
const submit=async()=>{if(!tokenValue.value)return;loading.value=true;error.value='';try{if(await checkTokenValidation(tokenValue.value))await router.push('/');else{failed.value=true;setTimeout(()=>failed.value=false,420)}}catch(e){error.value=e instanceof Error?e.message:'Ошибка входа'}finally{loading.value=false}}
</script>
