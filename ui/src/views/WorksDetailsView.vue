...
<template>
  <main v-if="currentProject" class="case" :class="{'case--light-background':isLightBackground}" :style="{backgroundColor:currentProject.details.theme.background||'#000',color:currentProject.details.theme.textColor||'#fff','--accent':currentProject.details.theme.accentColor||editorial?.accent||'#777'}">
    <section class="case-hero contained"><div class="case-intro"><h1>{{currentProject.info.title}}</h1><p v-if="currentProject.details.content[0]" v-html="richText(currentProject.details.content[0].text)"/><div v-if="currentProject.info.year || caseTags.length || projectLink" class="case-tags"><span v-if="currentProject.info.year" class="case-tag--accent">{{currentProject.info.year}}</span><span v-for="(tag,index) in caseTags" :key="tag" :class="{'case-tag--accent':index===0}">{{tag}}</span><a v-if="projectLink" class="case-tag--accent case-tag-link" :href="projectLink" target="_blank" rel="noopener noreferrer">{{getLanguageIso()==='ru'?'На сайт':getLanguageIso()==='sr'?'Веб-сајт':'Website'}}<i aria-hidden="true"/></a></div></div><div class="case-cover"><UiImage :images="currentProject.info.images.slice(0,1).map(image=>image.link)" :gallery="false" :always-on="true"/></div></section>
...
.case-tag-link{display:inline-flex;align-items:center;gap:4px;text-decoration:none;transition:filter .16s ease}
.case-tag-link:hover{filter:brightness(1.15)}
.case-tag-link:focus-visible{outline:2px solid currentColor;outline-offset:3px}
.case-tag-link i{width:16px;height:16px;background:currentColor;mask:url('/assets/icons/icon-ext-white.svg') center/contain no-repeat;-webkit-mask:url('/assets/icons/icon-ext-white.svg') center/contain no-repeat}
</style>
